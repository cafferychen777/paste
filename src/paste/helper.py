...
def apply_trsf(
    M: np.ndarray, translation: list[float], points: np.ndarray
) -> np.ndarray:
    """
...
def to_dense_array(value):
    """Return a dense NumPy representation of an expression matrix."""
    if scipy.sparse.issparse(value):
        return value.toarray()
    return np.asarray(value)


def extract_data_matrix(adata, representation):
    """Select the expression matrix or a named observation representation."""
    return adata.X if representation is None else adata.obsm[representation]
