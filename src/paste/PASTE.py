    ...
    **kwargs,
    ...
    if use_gpu:
        try:
            ...
            raise ImportError("GPU alignment requires PyTorch") from exc
    ...
    if G_init is not None:
        ...
        if isinstance(nx, ot.backend.TorchBackend):
            ...
            if use_gpu:
                G_init.cuda()
    pi, logw = ot.gromov.fused_gromov_wasserstein(
        M,
        D_A,
        D_B,
        a,
...
def center_align(
    ...
    pis_init: list[np.ndarray] | None = None,
    distributions=None,
    backend=None,
    use_gpu: bool = False,
    verbose: bool = False,
